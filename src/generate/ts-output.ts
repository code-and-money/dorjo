...
import { type Relation, relationsInSchema, definitionForRelationInSchema, crossTableTypesForTables, crossSchemaTypesForAllTables, crossSchemaTypesForSchemas } from "./tables";
...
const canaryVersion: SchemaVersionCanary["version"] = 104,
  versionCanary = `
// got a type error on schemaVersionCanary below? update by running \`npx zbs\`
...
It's been generated by Zbs as a custom type definition placeholder, and won't be overwritten
*/
`;
...
export const tsForConfig = async (config: CompleteConfig, debug: (s: string) => void) => {
  ...
  const ts =
    ...
    declareModule(
      "zbs/schema",
      `\nimport type * as db from 'zbs/db';\n` +
        (hasCustomTypes ? `import type * as c from 'zbs/custom';\n` : ``) +
        versionCanary +
        "\n\n" +
        schemaDefs.join("\n\n") +
